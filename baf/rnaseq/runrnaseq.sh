...
#1. bowtie
parallel "bowtie2 -p 4 -x $repeat_index -q {} --un {.}_genome.fastq -S {.}_repeat.sam" ::: $fasta
parallel "bowtie2 -p 4 -x $genome_index -q {.}_genome.fastq -S {.}_genome.sam" ::: $fasta
...
parallel "bedtools genomecov -ibam {.}_genome_sorted_rmdup.bam -bg > {.}_genome_rmdup.bedGraph; norm_bedGraph.pl {.}_genome_rmdup.bedGraph {.}_genome_rmdup_norm.bedGraph" ::: $fasta
