...
# from the CAGE bedgraphs that have been intersected with enhancers, take the reverse and forward peaks such that R < F and val(R)+val(F) are maximized
...
tss_plus = getPos("DRR003906_CTT_genome_plus_norm_tss.bedGraph")
tss_minus = getPos("DRR003906_CTT_genome_minus_norm_tss.bedGraph")
...
for tss in tss_plus:
	...
	if maxSignal < 1000 or bestSense==[]: continue # sense peak not found/not high enough, or lower limit for sense peaks
...
# tssToClass = {}
# tssToInfo = {}
ofile = open("DRR003906_CTT_cage_tss_peaks.txt",'w')
writer = csv.writer(ofile, 'textdialect')
...
ifile = open("DRR003906_CTT_cage_tss_peaks.txt",'r')
reader = csv.reader(ifile, 'textdialect')
ofile = open("mm9_tss_cage_centered_1kb.bed",'w')
writer = csv.writer(ofile, 'textdialect')
unfile = open("mm9_tss_cage_uncentered_1kb.bed",'w')
