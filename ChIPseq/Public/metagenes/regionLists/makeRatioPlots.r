...
# large boxplots
regionTypes = c("TSS_trans_paused","mES_SEindiv_noTSS_1kb","RY_enh")
comparisons = list()
...
pdf("ratio_heatmaps_with7sk_atac_selectchip.pdf",width=4,height=6)

for (i in 1:nrow(denom)) {
	d = as.character(denom[i,1])
	pheatmap(comparisons.df[[d]],main=d,scale="none",col=colorRamp(50),breaks=seq(-2.5,2.5,5/50))
	#for allchip (bigger)
...
regionTypes = c("TSS_trans_paused","RY_enh","mES_SEindiv_noTSS_1kb")
