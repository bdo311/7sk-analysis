...
for sample in samples:
	...
	leftFile.close()	
	rightFile = open("/home/raflynn/7SK/GROseq/Flynn/metagenes/" + sample + "comb_sense/bins/RY_enh_TR_right250/allchr_sorted.txt",'r')
...
for sample in samples:
	...
	for row in reader:
		...
		geneToInfo[name][2]=float(row[6])
	leftFile.close()	
	rightFile = open("/home/raflynn/7SK/GROseq/Flynn/metagenes/" + sample + "comb_sense/bins/SE_indiv_TR_right250/allchr_sorted.txt",'r')
	reader = csv.reader(rightFile, 'textdialect')
	for row in reader:
		name = row[3]
		geneToInfo[name][2]=float(row[6])
