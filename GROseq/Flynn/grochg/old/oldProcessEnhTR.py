...
def getHashMap(fn):
	os.chdir("/home/raflynn/7SK/GROseq/Flynn/metagenes/")
	ifile = open(fn, 'r')
...
def main():
	...
	for tm in treatments: #strandedness doesn't matter since txn goes in both directions for reg enhs
		print tm
		ctr_plus = getHashMap(tm + '_sense/bins/RY_enh_TR_center250/allchr_sorted.txt')
		ctr_minus = getHashMap(tm + '_antisense/bins/RY_enh_TR_center250/allchr_sorted.txt')
		left_plus = getHashMap(tm + '_sense/bins/RY_enh_TR_left250/allchr_sorted.txt')
		left_minus = getHashMap(tm + '_antisense/bins/RY_enh_TR_left250/allchr_sorted.txt')
		right_plus = getHashMap(tm + '_sense/bins/RY_enh_TR_right250/allchr_sorted.txt')
		right_minus = getHashMap(tm + '_antisense/bins/RY_enh_TR_right250/allchr_sorted.txt')
		
		tr = []
		for gene in ctr_plus:
			prox = (ctr_plus[gene] + ctr_minus[gene])/2
			#dist = 0.5*(left_plus[gene] + left_minus[gene] + right_plus[gene] + right_minus[gene])
			dist = 0.5*(left_minus[gene] + right_plus[gene])
	...
	ofile = open("enh_tr250_lol.txt", 'w')
